...
def _load_schema(schema_pb2_path):
...
def trainer_fn(hparams):
  ...
  Returns:
    A dict of the following:
      ...
      - eval_input_receiver_fn: Input function for eval.
  """
  schema = _load_schema(hparams.schema_path)
  ...
  train_spec = tf.estimator.TrainSpec(  # pylint: disable=g-long-lambda
      train_input_fn,
      max_steps=int(hparams.train_steps))
  ...
  eval_exporter = tfma.exporter.FinalExporter(name='chicago-taxi-eval', eval_input_receiver_fn=receiver_fn)
  eval_spec = tf.estimator.EvalSpec(
      eval_input_fn,
      steps=int(hparams.eval_steps),
      exporters=[exporter, eval_exporter],
      name='chicago-taxi-eval')

  run_config = tf.estimator.RunConfig(
      save_checkpoints_steps=999, keep_checkpoint_max=1)

  run_config = run_config.replace(model_dir=hparams.model_dir)
